...
--------주의할 점----------
--그룹함수 : 그룹함수와 일반 컬럼을 동시에 출력할 수 없음(오라클만) 
...
--주의할 점(그룹절에 사용할 컬럼만 SELECT절에 사용한다)
SELECT DEPARTMENT_ID, FIRST_NAME --ERROR
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID;
...
--개별적인 열과 그룹함수를 GROUP BY절 없이 사용할 수 없음
SELECT DEPARTMENT_ID, COUNT(*)
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID;


--그룹함수를 WHERE절에 적용할 수 없음
SELECT JOB_ID,AVG(SALARY)
FROM EMPLOYEES
WHERE AVG(SALARY) >= 10000 --ERR
GROUP BY JOB_ID;
--------------------------------------------------------------------------------
--그룹의 조건은 HAVING 절을 사용
SELECT JOB_ID, AVG(SALARY) 
FROM EMPLOYEES
GROUP BY JOB_ID
...
--부서아이디가 50번 이상인 부서를 그룹화시키고 그룹평균급여 5000이상만 출력
...
--ROLLUP - 각 그룹의 결과를 해당 함수로 계산한 총결과를 아래에 출력
...
--GROUPING() - 그룹절로 생성되면 0반환, 롤업 OR 큐브로 만들어지면 1반환
...
SELECT DEPARTMENT_ID, 
       TRUNC(AVG(SALARY*(1+COMMISSION_PCT)),2) AS 급여평균, 
       SUM(SALARY*(1+COMMISSION_PCT)) AS 급여합, 
       COUNT(*) AS 사원수
FROM EMPLOYEES
