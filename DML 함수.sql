--INSERT, UPDATE, DELETE 문을 작성하면 COMMIT 명령으로 실제 반영을 처리하는 작업이 필요함
--INSERT
...
ROLLBACK; --COMMIT 하기 전이라면 원상복구 가능
...
--EX3 : ID-200인 사람의 급여를 103번의 급여로 변경(서브쿼리)
UPDATE EMPS
...
SELECT * FROM DEPTS;
SELECT * FROM EMPS;
--EX1 - 삭제할 때는 꼭 PK를 이용해야 함
DELETE FROM EMPS WHERE EMPLOYEE_ID = 200;
--DELETE FROM EMPS WHERE SALARY >= 4000; 모든 행 삭제
...
--EMPLOYEE가 60번 부서를 사용하기 때문에 DEPARTMENTS 테이블의 DEPARTMENT_ID 60번을 삭제할 수 없음
